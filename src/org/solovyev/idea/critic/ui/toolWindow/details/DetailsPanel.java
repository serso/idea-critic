...
public class DetailsPanel extends SimpleToolWindowPanel {
	...
	private JPanel installActions() {
		...
		final AddCommentAction addCommentAction = new AddCommentAction(myReview, null, null,
				CriticBundle.message("critic.add.comment"), false);
		...
		final AddCommentAction replyToCommentAction = new AddCommentAction(myReview, null, null,
				CriticBundle.message("critic.reply"), true);
		replyToCommentAction.setContextComponent(myGeneralComments);
		actionGroup.add(replyToCommentAction);

		final ActionPopupMenu actionPopupMenu = ActionManager.getInstance().createActionPopupMenu(CriticBundle.message("critic.main.name"),
				actionGroup);
		final JPopupMenu popupMenu = actionPopupMenu.getComponent();
		myGeneralComments.setComponentPopupMenu(popupMenu);

		final Border border = IdeBorderFactory.createTitledBorder(CriticBundle.message("critic.general.comments"), false);
	...
	private JPanel createCommitsPane() {
		...
		commitColumnNames.add(CriticBundle.message("critic.commit"));
		commitColumnNames.add(CriticBundle.message("critic.author"));
		commitColumnNames.add(CriticBundle.message("critic.date"));
		...
		myCommitsTable = new JBTable(myCommitsModel) {
			...
			public TableCellRenderer getCellRenderer(int row, int column) {
				if (column == myCommitsTable.getColumnModel().getColumnIndex(CriticBundle.message("critic.commit")))
		...
		final ToolbarDecorator decorator = ToolbarDecorator.createDecorator(myCommitsTable).
				setToolbarPosition(ActionToolbarPosition.LEFT);
		if (myReview.getReviewers().contains(new User(CriticSettings.getInstance().USERNAME, null)) &&
				"Review".equals(myReview.getState())) {
			decorator.addExtraAction(new CompleteReviewAction(myReview, CriticBundle.message("critic.complete.review")));
		}
		decorator.addExtraAction(new AnActionButton(CriticBundle.message("critic.show.general.comments"),
				CriticBundle.message("critic.show.general.comments"), AllIcons.Actions.ShowChangesOnly) {
			@Override
	...
	class MyChangesBrowser extends ChangesBrowser {
		...
		public void calcData(DataKey key, DataSink sink) {
			if (key == CriticDataKeys.REVIEW) {
				sink.put(CriticDataKeys.REVIEW, myReview);
			}
