...
import org.solovyev.idea.critic.ui.toolWindow.details.CommentForm;
import org.solovyev.idea.critic.ui.toolWindow.details.CommentsTree;
import org.solovyev.idea.critic.ui.toolWindow.details.GeneralCommentsTree;
import org.solovyev.idea.critic.ui.toolWindow.diff.ReviewGutterIconRenderer;
...
public class AddCommentAction extends AnActionButton implements DumbAware {
	...
	private void addGeneralComment(@NotNull final Project project, DataContext dataContext) {
		...
		final JBPopup balloon = CommentBalloonBuilder.getNewCommentBalloon(commentForm, myIsReply ? CriticBundle
				.message("critic.new.reply.$0", commentForm.getParentComment().getPermId()) :
				CriticBundle
						.message("critic.new.comment.$0",
								myReview.getPermaId()));
	...
	private void addVersionedComment(@NotNull final Project project) {
		...
		final JBPopup balloon = CommentBalloonBuilder.getNewCommentBalloon(commentForm, myIsReply ?
				CriticBundle
						.message("critic.new.reply.$0", "Comment") :
				CriticBundle
						.message("critic.new.comment.$0", myFilePath));
		balloon.addListener(new JBPopupAdapter() {
			@Override
			public void onClosed(LightweightWindowEvent event) {
