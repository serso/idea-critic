...
public class CriticApi {
	...
	private static final Logger LOG = Logger.getInstance(CriticApi.class);
	...
	@NotNull
	public static BasicReview parseReview(@NotNull JsonObject item, @NotNull Project project, @NotNull CriticSession session)
			throws IOException {
		...
		if (reviewRaw.reviewItems != null) {
			for (ReviewItemRaw itemRaw : reviewRaw.reviewItems.reviewItem) {
				review.addReviewItem(createReviewItem(itemRaw, project, session));
			}
		}
	...
	private static ReviewItem createReviewItem(ReviewItemRaw raw, Project project, CriticSession session) throws IOException {
		if (raw.patchUrl != null) {
			return parsePatchReviewItem(raw, project, session);
		} else {
	...
	@Nullable
	private static ReviewItem parsePatchReviewItem(ReviewItemRaw raw, Project project, CriticSession session) throws IOException {
		final String id = raw.permId.id;
		final String toPath = raw.toPath;
		String patchUrl = raw.patchUrl;
		String file = session.downloadFile(patchUrl);
		...
		Map.Entry<String, VirtualFile> repoEntry = ContainerUtil.find(session.getRepositories().asIterable(),
	...
	@NotNull
	public static CriticVersionInfo parseVersion(@NotNull JsonObject object) {
		Version raw = gson.fromJson(object, Version.class);
		return new CriticVersionInfo(raw.releaseNumber, raw.buildDate);
	}
